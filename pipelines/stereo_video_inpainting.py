...
class StableVideoDiffusionInpaintingPipeline(DiffusionPipeline):
    ...
    def _encode_vae_frames(
        ...
        latent_list = []
        # Reduced n_frames_per_time from 5 to 3 for 4K to reduce VRAM
        ...
        if do_classifier_free_guidance:
            ...
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            frame_latents = torch.cat([negative_frame_latents, frame_latents])

        # duplicate image_latents for each generation per prompt, using mps friendly method
        frame_latents = frame_latents.repeat(num_videos_per_prompt, 1, 1, 1, 1)
    ...
    def __call__(
        ...
        # decode_chunk_size is passed from caller (UI or depth_crafter_ppl)
        # Default to a safe value if not specified
        decode_chunk_size = decode_chunk_size if decode_chunk_size is not None else 4
...
def load_inpainting_pipeline(
    ...
    # This processes the VAE in smaller slices instead of all at once
    try:
    ...
    # Enable gradient checkpointing on UNet to reduce VRAM (trades compute for memory)
    try:
